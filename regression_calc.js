...
function buildxy()  {
    ...
    with (Math) {
        ...
        //This is set to 2 because we are reading in 2 variables
        var numvariables = 2;

        //Runs through 12 rows
        for (var i = 0; i < 12; i++) {
            X[1][N] = testData.Dependents.Electricity[N];
            X[2][N] = testData.Dependents.Gas[N];

            Y[N] = testData.Independents.HDD[N];
            N++;
